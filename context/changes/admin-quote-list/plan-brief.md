...
**In scope:** draft create/update/delete endpoints; approval extended with an optional draft id and a required e-mail; quote list at `/admin`; editor moved to `/admin/quotes/new`; draft edit at `/admin/quotes/<id>`; read-only mode for approved quotes; a unit suite for the extracted payload service.
...
| 2. List at `/admin`, editor to `/admin/quotes/new` | FR-070 list, delete control, new-quote route                            | Moving `/admin` breaks the S-01 entry point if the new route is not wired first                                   |
| 3. Draft save + reopen for editing                 | C and U closed in the UI; e-mail gate on approval                       | Rehydration must restore tooth order and general-item ids or a reopened draft silently differs from the saved one |
