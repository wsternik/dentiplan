...
- [ ] 2.1 `e2e/home-page.spec.ts` passes at the three target widths and proves CTA/anchor destinations plus no 375 px overflow.
- [ ] 2.2 `npm run lint`, `npx astro check`, and `npm run build` pass.
- [ ] 2.3 `npm run test:e2e` passes, and the only E2E diff is the new home spec.
- [ ] 2.4 The home ships no prototype asset, external font request, session data, or client-side JavaScript for static decoration.

#### Manual

- [ ] 2.5 Captures at 1440, 768, and 375 px are ready for the non-blocking final visual review against the prototype.

### Phase 3: Entry and workspace surfaces

#### Automated

- [ ] 3.1 Auth E2E retains `Email`, `Password`, `Sign in`, no sign-up, and successful redirect.
- [ ] 3.2 Admin E2E retains the semantic table, strict `Nowy kosztorys` locator, draft row actions, and approval confirmation.
- [ ] 3.3 `npm run lint`, `npx astro check`, `npm test`, and `npm run build` pass.
- [ ] 3.4 `npm run test:e2e` passes without edits to existing specs.

#### Manual

- [ ] 3.5 Sign-in, populated/empty/error list states, approval, and not-found state use one visual hierarchy at desktop and 375 px.
