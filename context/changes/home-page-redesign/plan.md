...
| p2    | `beb3e6c` | `e2e/home.spec.ts` green; fails as intended when the sign-in link is duplicated (count 2) and when it is removed (count 0), restored after both. Full `npm run test:e2e` after `pkill -f "astro dev"; rm -rf node_modules/.vite`: 10 passed, 0 failed, first run, 15.9s. `git diff main...HEAD -- 'e2e/*.spec.ts'` shows only the added file.                                                                                                                                                                                                                                        |
