...
- Auth flow live (Supabase SSR cookie sessions): `/`, `/auth/signin`, `/auth/signup`, `/auth/confirm-email`, `/admin` (protected, redirects to `/auth/signin` when unauthenticated; `/dashboard` was starter scaffolding and was removed with the redesign; `/auth/signup` and `/auth/confirm-email` were starter scaffolding too and were removed on 2026-09-06 — the surgery runs on one hand-made account and the hosted project refuses sign-ups, see `context/archive/2026-09-06-remove-self-service-signup/`).
