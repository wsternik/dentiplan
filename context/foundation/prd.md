...
- FR-077: Dentystka can save the raw diagnosis note with a draft or approved quote and read it only in the authenticated admin panel. The note is not part of the patient-visible `content` tree, is never returned by the anonymous patient RPC, and is never displayed on the patient page. Priority: must-have
- FR-078: While the system fills the form from a diagnosis note, the editor visibly communicates that it is busy and prevents edits to the form until processing finishes; the page remains scrollable. Priority: must-have
