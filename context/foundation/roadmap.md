...
- **Blockers:** —
- **Unknowns:**
  - **Wybór dostawcy LLM** (Anthropic / OpenAI / OpenRouter / lokalny). Wymagania: polskojęzyczna jakość parsowania nazw zabiegów, strukturyzowane wyjście (JSON schema / tool use), niski koszt per request, brak retencji lub akceptowalne SLA prywatności. PRD Open Q #12 z shape-notes; `tech-stack.md` świadomie odłożył tę decyzję. — Owner: implementacja (dev). Block: **yes** (bez wybranego dostawcy nie da się napisać klienta).
...
| S-02       | llm-parsing-prefill                  | LLM pre-fills the new-quote form from pasted diagnosis text           | no                    | Blocked: wybór dostawcy LLM (PRD Open Q #12) musi być zamknięty przed planowaniem.         |
...
1. **Wybór dostawcy LLM (OpenAI / Anthropic / OpenRouter / lokalny model)** — wymagania: polskojęzyczna jakość parsowania, strukturyzowane wyjście (JSON schema / tool use), niski koszt per request, akceptowalna polityka retencji (mitigacja: bez PII w prompcie). PRD Open Q #12, zostawione przez `tech-stack.md` jako decyzja do podjęcia teraz. — Owner: dev. Block: `S-02`.
