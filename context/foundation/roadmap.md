...
- **Frontend:** present — Astro 6 SSR + React 19 islands + Tailwind 4 + shadcn/ui ("new-york"); pages w `src/pages/` (`index.astro`, `dashboard.astro`, `auth/{signin,signup,confirm-email}.astro`); jeden komponent UI shadcn (`button.tsx`). _(Stan z 25 V; `dashboard.astro` już nie istnieje — patrz aktualizacja niżej.)_
  **Aktualizacja 9 IX (`home-visual-system`):** starter pozostaje usunięty (`dashboard.astro`, `Welcome.astro`, `LibBadge.astro`), a wspólne tokeny prowadzą przez stronę główną, logowanie, panel i dokument pacjenta. Dwa kroje z npm — Manrope dla tekstu roboczego i Fraunces dla hierarchii redakcyjnej — są lokalne, bez font CDN. Publiczna strona główna jest statyczna (bez danych sesji i dekoracyjnego JavaScriptu); jej CTA i kotwice są chronione E2E na desktopie, tablecie i telefonie. Paleta nadal rezerwuje nasycony kolor dla znaczenia klinicznego, a status ma także niekolorowy sygnał, który zachowuje sens na wydruku.
...
- **Wspólny system wizualny i strona główna: dentystka widzi spójne wejście do DentiPlan, a publiczna strona wyjaśnia porównanie wariantów i prowadzi do logowania bez klientowego runtime.** — Delivered 2026-09-09 as `home-visual-system`. Lesson: kontrakt wydruku trzeba mierzyć w szerokości papieru; znaczenie kliniczne nie może zależeć od tła ani samego koloru.
