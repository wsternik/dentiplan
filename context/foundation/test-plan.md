...
| 11  | The note prefill proposes a visit split and an urgency the diagnosis text does not support; because it arrives complete and confident it is approved unread, and the patient gets a schedule nobody decided. Approved quotes are immutable, so the correction is a new quote and a new link.                                                                 | High   | Medium     | `context/changes/visit-planning-prefill/plan.md`; PRD FR-011, FR-012, FR-014, FR-015                                                                                                                                                                           |
| 10  | A prompt or default-model change silently makes the prefill worse at reading diagnosis notes, so the dentystka must correct more teeth, items, warnings and visit groups by hand — or approves a plausible-looking mistake because application mapper tests remain green.                                                                                    | Medium | Medium     | `context/changes/prompt-evals/research.md`; risk #7 starts after the provider boundary and therefore cannot measure whether the live model recognized the note correctly                                                                                       |
...
| 6   | An unauthenticated request reaches the admin panel, or the dentist — the single operator — is locked out of it.                                                                                                                                                                                                                                              | High   | Low        | PRD FR-003, Access Control; roadmap S-05 (`ready`, not yet implemented); until 2026-09-06 the sign-in page also linked to a working `/auth/signup` — see `context/archive/2026-09-06-remove-self-service-signup/`, now guarded by `e2e/auth-no-signup.spec.ts` |
| 8   | The drawing and the written list on the patient page describe two different mouths — a tooth marked on the chart that no list names, or one the chart colours as urgent while the list prints "łagodne". The patient is looking at a picture of her own teeth, so when the two disagree the picture is what she believes.                                    | Medium | Medium     | roadmap S-06 / PRD FR-074 – FR-076; `context/changes/tooth-chart-visualization/plan.md` (the chart is a second, parallel rendering of `content.teeth` beside FR-061's list)                                                                                    |
| 13  | A saved raw diagnosis note, needed by the dentystka in the panel, leaks through the anonymous patient response or the `get_quote_by_token` RPC. The note may contain medical free text, so one leak exposes more than the patient needs to see.                                                                                                              | High   | Medium     | PRD FR-077; `context/changes/quote-form-polish/plan.md`; the patient RPC returns `content` verbatim and only an explicit top-level whitelist protects dedicated admin columns                                                                                  |
| 14  | The public home becomes a decorative second application: its primary path no longer reaches sign-in, it overflows on a phone, or a visual refresh changes the semantic controls that the dentist uses in the authenticated flow.                                                                                                                             | Medium | Medium     | `context/changes/home-visual-system/plan.md`; the home, auth and admin surfaces share tokens while the public page intentionally has no client runtime                                                                                                         |
...
| #11  | Every value the split was not read from the note — an inferred urgency, the model's reasoning for the grouping — is named in a warning she reads before approving, and no free-text string the model wrote reaches `content`, which the patient link serves verbatim.                                                     | That a green mapping test proves the split is _sensible_. It proves only that it is _labelled_: numbering, ordering and naming are ours and testable, but whether tooth 36 belongs in visit 1 is a clinical judgement the tests cannot hold — she can. | Where the model's proposal stops being a proposal and becomes the form's visits, and which of the visible sentences are composed by code rather than copied. | unit, on recorded model answers                 | Asserting only that the visits came back non-empty. The absence assertion (no model prose in `content`) is the half that catches the regression, and it has to be positive: every label is a member of the closed dictionary. |
| #10  | Before a production prompt or default-model change, the same source-controlled synthetic notes and fixed atoms are run uncached against the candidate matrix; raw provider answers must retain safety eligibility and win the predeclared quality-first ranking.                                                          | That risk #7's green mapper tests prove the provider still reads notes well. They start from recorded model answers and deliberately never cross the paid, non-deterministic provider boundary.                                                        | The live prompt builder, wire schema, provider model/options and the boundary before `mapParsedDiagnosis` repairs or drops output.                           | paid manual Promptfoo evaluation                | Running a real provider call in CI, scoring mapper-repaired output, changing an oracle after seeing a response, or selecting from cost/latency before safety and quality.                                                     |
...
| #12  | The printed estimate keeps the two treatment variants side by side and the deferred teeth still look deferred, measured at the width a printer actually has (~717px A4) rather than at a screen viewport with print media emulated.                                                                                       | That emulating the print medium is enough to test print. It changes which CSS applies; it does not change the width a breakpoint is evaluated against — so a `md:` (768px) two-column layout looks fine in a 1280px capture and stacks on paper.       | Which breakpoints the printed layout depends on, and which distinctions rest on a background image (print drops those by default).                           | e2e at print width                              | Asserting the print stylesheet contains the rule instead of asserting the laid-out page: the rule that was missing was the one nobody had written, so that check passes before and after the bug.                             |
| #8   | For an approved quote, every tooth the chart marks is named by one of the page's lists under the status the chart draws for it, and every tooth those lists name is marked on the chart — nothing in one and not the other — with the urgency hue the chart fills matching the one the list prints beside the same tooth. | That "both read `content.teeth`" makes them agree. They are two independent mappings — `toChartTeeth` against the three section filters — and the day a fourth status arrives one of them gets updated and the other does not.                         | Which section each status routes a tooth into, and where the chart's urgency hue and the list's urgency mark come from — two different tables.               | e2e on the rendered patient page                | Asserting that the chart draws 32 teeth. That passes whether or not a single mark agrees with the list: a count is not an agreement.                                                                                          |
| #13  | A persisted diagnosis note appears in the authenticated draft and approved views, but neither the `/p/<token>` response bytes nor the anonymous `get_quote_by_token` response contains it.                                                                                                                                | That the patient component does not render the note. The page can still leak it through server-rendered HTML, serialised props, or the RPC response.                                                                                                   | The three write routes, the admin-only quote read, `QuotePayloadSchema` / `buildQuoteContent`, and the RPC's explicit return columns.                        | e2e on both anonymous response surfaces         | Testing only for absence without first proving the note was saved and visible in the authenticated panel; a failed save would make that assertion pass.                                                                       |
| #14  | At 1440, 768 and 375 px the public home has one primary heading, stable anchor targets and practice CTAs that reach sign-in; at 375 px the document does not overflow, while the existing authenticated-surface semantics remain green.                                                                                   | That matching the approved composition visually protects navigation and product controls. Decorative similarity says nothing about link destinations, narrow containment or accessible names shared with the workspace.                                | The public home's anchors and CTAs, the 375 px document width, and the existing auth/admin E2E contracts that consume the shared visual tokens.              | e2e for home plus unchanged surface contracts   | A pixel snapshot of the home. It is brittle around decoration and can pass while a CTA points at the wrong route or a shared control loses its accessible name.                                                               |
...
| 6   | Visit proposal boundary       | Prove the split the model proposes is numbered, named and warned about by us, not by it              | #11           | unit              | complete    | `context/changes/visit-planning-prefill/` (`src/lib/llm/visits.test.ts`, `parse-diagnosis.test.ts`, `prompt.test.ts`)   |
| 7   | Chart / list agreement        | Prove the drawing and the written list on the patient page describe the same mouth                   | #8            | e2e               | complete    | `context/changes/tooth-chart-visualization/` (`e2e/tooth-chart.spec.ts`)                                                |
| 8   | Saved diagnosis-note boundary | Prove a saved raw note stays in the authenticated panel and out of both anonymous patient surfaces   | #13           | e2e               | complete    | `context/changes/quote-form-polish/` (`e2e/diagnosis-note-privacy.spec.ts`)                                             |
| 10  | Public entry contract         | Prove the responsive static home retains its CTA and anchor destinations without horizontal overflow | #14           | e2e               | complete    | `context/changes/home-visual-system/` (`e2e/home-page.spec.ts`)                                                         |
| 9   | Provider-reading regression   | Prove the selected prompt/model reads the frozen synthetic corpus before production adopts it        | #10           | manual Promptfoo  | complete    | `context/changes/prompt-evals/` (`evals/README.md`, `evals/prefill/`)                                                   |
...
- **Visual snapshots of pages and components** — they break on every style change and catch little. Re-evaluate if the patient page ever ships a graphical tooth chart. (Source: Phase 2 interview Q5.) **Re-evaluated 2026-09-06**, when S-06 shipped exactly that chart: still excluded. Risk #8 asserts that the drawing says the same thing as the list, which is behaviour and survives a restyle; whether the drawing _looks_ right stays a manual check, per the same slice's plan.
...
- Strategy (§1–§5) last reviewed: 2026-09-08 (risk #10 added for live provider-reading regressions; risk #13 covers the saved diagnosis-note boundary; risk #8 covers the tooth chart; risk #11 covers the visit-split prefill; risk #6 was re-read when self-service sign-up was removed — impact unchanged, likelihood lower, and the absence of the door is now a test rather than a convention)
