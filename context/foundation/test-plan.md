...
- Risk #6 coverage extended: 2026-09-08 (`home-page-redesign` rewrote `/`, the one page whose job is to explain the product to a stranger and therefore the one where a sign-up link reads like an improvement; `e2e/home.spec.ts` counts the links rather than merely finding the sign-in one. Strategy unchanged: same risk, same impact, one more surface asserted.)
- Strategy (§1–§5) last reviewed: 2026-09-07 (risk #13 added for the saved diagnosis-note boundary; risk #8 covers the tooth chart; risk #11 covers the visit-split prefill; risk #6 was re-read when self-service sign-up was removed — impact unchanged, likelihood lower, and the absence of the door is now a test rather than a convention)
